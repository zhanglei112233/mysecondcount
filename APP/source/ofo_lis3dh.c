...
AxesRaw_t LIS3DH_XYZRaw[LIS3DH_BUFF_LEN] = {0};
...
//	;
  return my_twi_read(LIS_3DH_ADDRES,Reg,1,Data);
}
...
u8_t LIS3DH_WriteReg(u8_t WriteAddr, u8_t Data) {
  ...
  //i.e. SPI_Mems_Write_Reg(WriteAddr, Data); 
	//	;
  return my_twi_write(LIS_3DH_ADDRES,WriteAddr,1,&Data);
}
...
//	response = LIS3DH_SetODR(LIS3DH_ODR_1Hz);
	...
	//set Int1
	response = LIS3DH_SetInt1Pin(LIS3DH_WTM_ON_INT1_ENABLE); //22h  0x04
	
	//Sets Watermark Value
	response = LIS3DH_SetWaterMark(LIS3DH_BUFF_LEN);     //2E 0x59
	
	//Enable FIFO
	response = LIS3DH_FIFOModeEnable(LIS3DH_FIFO_STREAM_MODE);
...
{
	AxesRaw_t data1;
	LIS3DH_GetAccAxesRaw(&data1);
//	 dprintf( "X=%6d Y=%6d Z=%6d \r\n", data1.AXIS_X, data1.AXIS_Y, data1.AXIS_Z);
}

void test(void)
{
	AxesRaw_t data[LIS3DH_BUFF_LEN];		
	uint8_t i ;
	for	(i = 0;i<LIS3DH_BUFF_LEN;i++)
	{
	   LIS3DH_GetAccAxesRaw(&(data[i]));
//	 printf( "X=%6d Y=%6d Z=%6d \r\n", data[i].AXIS_X, data[i].AXIS_Y, data[i].AXIS_Z);
	}
}

void LIS3DH_GetAccXYZRaw(AxesRaw_t *Data)
{
  for	(uint8_t i = 0;i<LIS3DH_BUFF_LEN;i++)
	{
		 LIS3DH_GetAccAxesRaw(&(Data[i]));
//	   printf( "X=%6d Y=%6d Z=%6d \r\n", Data[i].AXIS_X, Data[i].AXIS_Y, Data[i].AXIS_Z);
		 nrf_delay_ms(1);
...
uint8_t BikeRunStopDec(AxesRaw_t *Data)
{
//	uint8_t response = 0;
	uint8_t State_Flag = 0;
//	AxesRaw_t data[LIS3DH_BUFF_LEN] = {0};		
...
//	}
  ...
  for	(uint8_t i = 1;i<LIS3DH_BUFF_LEN;i++)
	{
		 if((abs(Data[i].AXIS_X - Data[i-1].AXIS_X) > 512) || (abs(Data[i].AXIS_Y - Data[i-1].AXIS_Y) > 512) || (abs(Data[i].AXIS_Y - Data[i-1].AXIS_Y) > 512))
		 {
			 State_Flag++;
		 }		 
	}
...
uint8_t BikeTiltDec(AxesRaw_t *Data)
{
//	uint8_t response = 0;
	uint8_t State_Flag = 0;
//	AxesRaw_t data[LIS3DH_BUFF_LEN] = {0};		
...
//	}
  ...
  for	(uint8_t i = 0;i<LIS3DH_BUFF_LEN;i++)
	{
		 if((Data[i].AXIS_Y < 2000) && (Data[i].AXIS_Y > (-2000)) && (2096 < Data[i].AXIS_Z) && (Data[i].AXIS_Z < 4096))
		 {
			 State_Flag++;
		 }	
	}	
...
void BikeState(uint8_t sel)
{	
	if(sel == 0)
	{
	  BikeRunStopDec(LIS3DH_XYZRaw);
	}
	else if(sel == 1)
	{
    BikeTiltDec(LIS3DH_XYZRaw);
	}
}
